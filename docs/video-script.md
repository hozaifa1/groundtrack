# Groundtrack: the 3-minute video script
...
### Shot 1 [0:00 to 0:14] | Terminal: git provenance
...
### Shot 2 [0:14 to 0:25] | Browser: console hero and stat cards
...
- **Director Note:** This is the line the whole video hangs on. Speak steadily through "neither of us" and pause briefly before scrolling.
...
### Shot 4 [0:41 to 1:00] | Browser: the sealed grader
...
### Shot 5 [1:00 to 1:32] | Browser: walkthrough of the rounds
...
### Shot 6 [1:32 to 1:54] | Browser: the final numbers
...
### Shot 7 [1:54 to 2:08] | GitHub / Browser: the generalisation finding *(cut this first if rehearsal runs past 2:45)*
...
### Shot 9 [2:30 to 2:55] | Terminal: live verification and close
...
- **Spoken Line:**
  ...
  > / *Most submissions ask you to trust the pitch. This one hands you the commands.*
...
- **Did IBM Bob find the parameters, or did you?**  
  An offline parameter search on the development split found the two constants. Bob authored the engine code and the reasoning around it, and the held-out grader judged the result. `README.md` and `docs/parameter-search.md` both record that division of labour.
- **Is a held-out split not standard ML practice?**  
  Data splits are standard in model training. Here the split governs an autonomous coding agent's revisions: Bob writes the code changes and can reach neither the evaluation set nor the metric. Across 432 configurations, the visible development scores and the holdout scores showed zero correlation, so tuning against the split Bob can see buys nothing on the split it cannot.
- **Recall dropped from 25 to 19. Why is this preferable?**  
  On the held-out recordings the engine finds 19 of 35 faults, and false alarms dropped from 128 to 7. For a small team triaging by hand, that takes 121 items out of the queue. Operators who need the recall back can move the two constants in `engine/detect.py` with the included search scripts.
