...
class Bench:
    ...
    def z(self, chan: str, rolling_window: int) -> np.ndarray:
        key = (chan, rolling_window)
        if key not in self._z_cache:
            self._z_cache[key] = z_scores(self.values[chan], rolling_window)
    ...
    def evaluate(self, cfg: dict) -> dict:
        ...
        for chan, truth in self.truth:
            if chan not in cache:
                z = self.z(chan, cfg.get("rolling_window", 100))
